        ...
        try {
            ...
            if (category) {
                const response = await fetch(
                    `/api/products?category=${category}&exclude=${item.slug || ""}`
                )
                if (response.ok) {
        ...
        switch (type) {
            ...
            case "alternative":
                ...
                if (accepted && options?.currentItemPrice && options?.alternativePrice != null) {
                    ...
                    stats.alternative.savings += Math.max(
                        0,
                        options.currentItemPrice - options.alternativePrice
                    )
                }
                break
            case "block":
