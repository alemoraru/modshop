...
export default function CartPage() {
    ...
    const processCheckout = () => {
        ...
        const order = {
            ...
            items: items.map(({ slug, title, price, quantity, image }) => ({
                slug,
                title,
                price,
                quantity,
    ...
    const handleNudgeAccept = (nudgeType: string) => {
        ...
        if (nudgeType === "alternative" && currentNudge?.data) {
            ...
            if (alternativeData.isAlreadyCheapest) {
                if (originalItem) {
                    ...
                    setNotificationMessage(
                        `💰 Great thinking! You saved €${savedAmount.toFixed(2)} by removing "${originalItem.title}" from your cart.`
                    )
                    setShowNotification(true)
                }
                setCurrentNudge(null)
            ...
            if (
                originalItem &&
                alternativeData.alternativeProduct &&
                alternativeData.alternativePrice
            ) {
                removeItem(originalItem.slug)
                ...
                addItem({
                    ...
                    category:
                        alternativeData.alternativeCategory || originalItem.category || "general",
                })

                setNotificationType("success")
                setNotificationMessage(
                    `Switched to ${alternativeData.alternativeProduct}! You saved €${(alternativeData.currentPrice! - alternativeData.alternativePrice).toFixed(2)}.`
                )
                setShowNotification(true)
    ...
    return (
        <main className="bg-white text-gray-900">
            <section className="p-6 max-w-5xl mx-auto">
                ...
                {items.length === 0 ? (
                    <div className="flex flex-col items-center gap-8 py-20 bg-gradient-to-br from-blue-50 to-gray-100 rounded-2xl shadow-inner">
                        <div className="flex flex-col items-center">
                            ...
                            <p className="text-gray-500 mb-4 max-w-xs text-center">
                                ...
                                discover great products tailored for you! Be mindful of your
                                purchases!
                            </p>
                        </div>
                        <div className="flex flex-col w-full max-w-xs gap-3 sm:flex-row sm:justify-center sm:max-w-none">
                            <Link
                                href="/"
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="mt-8 space-y-4">
                            <div className="text-xl font-bold text-gray-800 flex justify-between items-center flex-wrap gap-4">
                                            ...
                                            onClick={
                                                user
                                                    ...
                                                    : () => {
                                                          setNotificationType("warning")
                                                          setNotificationMessage(
                                                              "Please log in to use nudges or continue with purchases."
                                                          )
                                                          setShowNotification(true)
                                                      }
                                            }
            ...
            {currentNudge?.type === "alternative" && (
                <CheaperAlternativeNudge
                    ...
                    alternativeProduct={
                        currentNudge.data?.alternativeProduct || "Basic Alternative"
                    }
